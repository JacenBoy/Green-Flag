...
// Define the URI for the NASCAR API
const scoringFeed = "https://cf.nascar.com/live/feeds/live-feed.json";
...
(async () => {
  ...
  while (true) {
    try {
      ...
      const scoring = await fetch(scoringFeed).then(res => res.json());
      // Fetch the lap notes or return an empty object if there are none
      const racenotes = await fetch(`https://cf.nascar.com/cacher/${year}/${scoring.series_id}/${scoring.race_id}/lap-notes.json`).then(res => res.json()).catch(err => {return {laps: {}}});
